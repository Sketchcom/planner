...
def main_app():
    ...
    # Step 4: Upload CSV for Task Data
    st.subheader("Upload CSV for Previous Day Tasks (Optional)")
    ...
    if uploaded_file is not None:
        ...
        # If CSV file is valid, fill the task management fields automatically
        if "Project" in uploaded_df.columns and "Task" in uploaded_df.columns and "Responsible" in uploaded_df.columns:
            tasks_df = uploaded_df
            st.success("CSV file uploaded successfully. Fields are prefilled.")
        else:
            st.error("Uploaded CSV must have 'Project', 'Task', and 'Responsible' columns.")
    ...
    # Step 6: Generate PDF and CSV (with IST timezone)
    if not tasks_df.empty:
        if st.button("Generate PDF and CSV"):
            class PDF(FPDF):
                def header(self):
            ...
            # Save CSV of the tasks for download
            csv_data = tasks_df.to_csv(index=False)
            st.download_button("Download CSV", data=csv_data, file_name=f"tasks-{formatted_date}.csv", mime="text/csv")
    else:
